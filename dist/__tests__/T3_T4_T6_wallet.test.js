...
describe('T3, T4, T6 - Wallet Operations', () => {
    let app;
    const testPlayerId = (0, unique_1.uniqueId)('wallet');
    const testWalletId = (0, uuid_1.v4)();
    beforeAll(async () => {
        app = createApp();
      ...
      VALUES ($1, $2, $3, $4, $5)
    `, [testPlayerId, 'wallet_test_user', (0, unique_1.uniqueEmail)('wallet'), 'hash', 'active']);
        await (0, database_1.query)(`
    ...
    afterAll(async () => {
        ...
        await (0, database_1.query)('DELETE FROM idempotency_keys WHERE key LIKE $1', ['%test-wallet%']);
    ...
    describe('T3 - Fondi insufficienti', () => {
        it('should reject debit when amount exceeds balance', async () => {
            ...
            const newWalletResult = await (0, database_1.query)('SELECT balance FROM user_wallets WHERE user_id = $1', [testPlayerId]);
            ...
            const txResult = await (0, database_1.query)(`
      ...
      `, [testPlayerId, parseFloat(excessAmount)]);
        ...
        it('should reject debit that would make balance negative', async () => {
            ...
            await (0, database_1.query)('UPDATE user_wallets SET balance = $1 WHERE user_id = $2', [10.00, testPlayerId]);
            const response = await (0, supertest_1.default)(app)
                .post('/wallet/debit')
                .set('X-Correlation-Id', (0, uuid_1.v4)())
                .set('Authorization', 'Bearer test_rgs_key')
    ...
    describe('T4 - Currency binding', () => {
        it('should reject debit with mismatched currency', async () => {
            ...
            const walletResult = await (0, database_1.query)('SELECT balance, currency FROM user_wallets WHERE user_id = $1', [testPlayerId]);
    ...
    describe('T6 - Debit-Credit round-trip', () => {
        it('should maintain balance integrity in round-trip', async () => {
            ...
            await (0, database_1.query)('UPDATE user_wallets SET balance = $1 WHERE user_id = $2', [initialBalance, testPlayerId]);
            const amount = '75.50';
            // Step 1: Debit
            ...
            const creditResponse = await (0, supertest_1.default)(app)
                .post('/wallet/credit')
                .set('X-Correlation-Id', (0, uuid_1.v4)())
                .set('Authorization', 'Bearer test_rgs_key')
                .set('Idempotency-Key', (0, uuid_1.v4)())
      ...
      `, [testPlayerId, parseFloat(amount)]);
        ...
        it('should handle multiple round-trips correctly', async () => {
            ...
            const finalResult = await (0, database_1.query)('SELECT balance FROM user_wallets WHERE user_id = $1', [testPlayerId]);
            expect(parseFloat(finalResult.rows[0].balance)).toBeCloseTo(expectedBalance, 2);
        });
    });
});
//# sourceMappingURL=T3_T4_T6_wallet.test.js.map