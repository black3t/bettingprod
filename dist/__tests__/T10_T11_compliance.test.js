...
const testLogCapture = [];
const mockTransport = new winston_1.default.transports.Console({
...
describe('T10, T11 - Compliance Tests', () => {
    let app;
    const testPlayerId = (0, unique_1.uniqueId)('compliance');
    beforeAll(async () => {
        app = createApp();
      ...
      VALUES ($1, $2, $3, $4, $5)
    `, [testPlayerId, 'compliance_test_user', (0, unique_1.uniqueEmail)('compliance'), 'hash', 'active']);
        await (0, database_1.query)(`
    ...
    describe('T10 - Logging & GDPR', () => {
      ...
      `;
            ...
            try {
                const result = await (0, database_1.query)(retentionQuery);
    ...
    describe('T11 - Arrotondamenti', () => {
        ...
        it('should store amounts consistently in minor units', async () => {
            ...
            const response = await (0, supertest_1.default)(app)
                .post('/wallet/debit')
                .set('X-Correlation-Id', (0, uuid_1.v4)())
                .set('Idempotency-Key', (0, uuid_1.v4)()) // Nell'header come da Bibbia §144!
                .set('Authorization', 'Bearer test_rgs_key')
                .send({
                playerId: testPlayerId,
                amount,
...
//# sourceMappingURL=T10_T11_compliance.test.js.map