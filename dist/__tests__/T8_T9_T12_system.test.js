...
describe('T8, T9, T12 - System Tests', () => {
    ...
    describe('T8 - Timeout/Busy path', () => {
        it('should return RW-RGS-008 on timeout', async () => {
            // Mock slow database
            const originalQuery = database_1.query;
        ...
        it('should return RW-SYS-000 on system error', async () => {
            // Mock database error
            jest.spyOn(require('../config/database'), 'query').mockRejectedValueOnce(new Error('Database connection lost'));
            const response = await (0, supertest_1.default)(app)
                .post('/wallet/credit')
                .set('X-Correlation-Id', (0, uuid_1.v4)())
                .set('Idempotency-Key', (0, uuid_1.v4)())
    ...
    describe('T9 - Webhook delivery', () => {
        ...
        it('should send playerExcluded webhook with correct payload', async () => {
            ...
            if (process.env.RGS_WEBHOOK_URL) {
                expect(mockedAxios.post).toHaveBeenCalledWith(expect.stringContaining('/playerExcluded'), expect.objectContaining({
        ...
        it('should send balanceChanged webhook with correct payload', async () => {
            ...
            expect(response.status).toBe(200);
            if (process.env.RGS_WEBHOOK_URL) {
                expect(mockedAxios.post).toHaveBeenCalledWith(expect.stringContaining('/balanceChanged'), expect.objectContaining({
                    playerId: expect.any(String),
                    newBalance: '1500.00'
                }), expect.any(Object));
            }
    ...
    describe('T12 - Health/Readiness', () => {
        ...
        it('should handle Redis down gracefully in non-SQLite mode', async () => {
            ...
            // Mock Redis failure
            if (redis_1.redis) {
                jest.spyOn(redis_1.redis, 'ping').mockRejectedValueOnce(new Error('Redis down'));
            }
            const response = await (0, supertest_1.default)(app)
                .get('/health');
            expect(response.status).toBe(503);
            ...
            if (redis_1.redis) {
                jest.spyOn(redis_1.redis, 'ping').mockRestore();
            }
        });
    });
});
//# sourceMappingURL=T8_T9_T12_system.test.js.map