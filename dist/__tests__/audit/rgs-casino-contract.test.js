...
describe('RGS⇄Casino Contract Tests (21 endpoints)', () => {
    ...
    beforeEach(() => {
        ...
        app.use('/rg', rgRoutes_1.default);
        app.use('/rgs', webhookRoutes_1.default);
        app.use('/session', rgsGameRoutes_1.default);
        app.use('/round', rgsGameRoutes_1.default);
        app.use('/health', healthRoutes_1.default);
        app.use('/admin/health', healthRoutes_1.default);
        app.use('/', rgsGameRoutes_1.default); // For /rollback alias
    });
    describe('Casino Endpoints (RGS → Casino)', () => {
        ...
        test('3. POST /wallet/cancel', async () => {
            const res = await (0, supertest_1.default)(app)
                ...
                .set('X-Correlation-Id', correlationId)
                .set('Idempotency-Key', (0, uuid_1.v4)())
                .send({ playerId, transactionId: (0, uuid_1.v4)(), amount: '10.50' });
            expect([200, 422]).toContain(res.status);
            if (res.status === 200) {
                expect(res.body).toHaveProperty('status');
            }
    ...
    describe('RGS Endpoints', () => {
        test('15. POST /session/validate', async () => {
            const res = await (0, supertest_1.default)(app)
                ...
                .set('Authorization', 'Bearer test_rgs_key')
                .set('X-Correlation-Id', correlationId)
                .send({ sessionId: (0, uuid_1.v4)(), playerId });
        ...
        test('17. POST /round/start', async () => {
            const res = await (0, supertest_1.default)(app)
                ...
                .set('X-Correlation-Id', correlationId)
                .set('Idempotency-Key', (0, uuid_1.v4)())
                .send({ playerId, gameId: 'rawwar', roundId: (0, uuid_1.v4)() });
            expect([200, 422]).toContain(res.status);
        });
        test('18. POST /round/end', async () => {
            const res = await (0, supertest_1.default)(app)
                ...
                .set('X-Correlation-Id', correlationId)
                .set('Idempotency-Key', (0, uuid_1.v4)())
                .send({ playerId, roundId: (0, uuid_1.v4)() });
            expect([200, 422]).toContain(res.status);
        });
        test('19. POST /round/cancel', async () => {
            const res = await (0, supertest_1.default)(app)
                ...
                .set('X-Correlation-Id', correlationId)
                .set('Idempotency-Key', (0, uuid_1.v4)())
                .send({ playerId, roundId: (0, uuid_1.v4)(), reason: 'user_request' });
            expect([200, 422]).toContain(res.status);
        });
        test('20. POST /round/rollback (canonical)', async () => {
            const res = await (0, supertest_1.default)(app)
                ...
                .set('X-Correlation-Id', correlationId)
                .set('Idempotency-Key', (0, uuid_1.v4)())
                .send({ playerId, transactionId: (0, uuid_1.v4)(), amount: '10.00' });
            expect([200, 422]).toContain(res.status);
        });
    ...
    describe('Idempotency Tests', () => {
        test('Wallet cancel idempotency replay', async () => {
            const idempKey = (0, uuid_1.v4)();
            const payload = { playerId, transactionId: (0, uuid_1.v4)(), amount: '10.50' };
            // First request
            ...
            const res2 = await (0, supertest_1.default)(app)
                .post('/wallet/cancel')
                .set('Authorization', 'Bearer test_rgs_key')
                .set('X-Correlation-Id', (0, uuid_1.v4)()) // Different correlation ID
                .set('Idempotency-Key', idempKey)
                .send(payload);
        ...
        test('Round cancel idempotency replay', async () => {
            const idempKey = (0, uuid_1.v4)();
            const payload = { playerId, roundId: (0, uuid_1.v4)(), reason: 'test' };
            // First request
            const res1 = await (0, supertest_1.default)(app)
                .post('/round/cancel')
                .set('Authorization', 'Bearer test_rgs_key')
...
//# sourceMappingURL=rgs-casino-contract.test.js.map