...
describe('E2E RGS-Casino Integration Suite', () => {
    ...
    afterAll(async () => {
        if (casinoServer)
            await new Promise((resolve) => casinoServer.close(resolve));
        if (rgsServer)
            await new Promise((resolve) => rgsServer.close(resolve));
    });
    describe('TASK A - E2E Flow with Concurrency', () => {
        it('should seed player via Casino admin endpoint', async () => {
            const response = await (0, supertest_1.default)(casinoApp)
        ...
        it('should generate RGS auth token', async () => {
            const response = await (0, supertest_1.default)(rgsApp)
        ...
        it('should handle concurrent debits with idempotency', async () => {
            ...
            const idempotentKey = (0, uuid_1.v4)();
            const idempotentTxId = (0, uuid_1.v4)();
            const roundId = (0, uuid_1.v4)();
            for (let i = 0; i < 3; i++) {
                requests.push((0, supertest_1.default)(rgsApp)
            ...
            const balanceCheck = await (0, supertest_1.default)(casinoApp)
    ...
    describe('TASK B - Outbox & Retry', () => {
        it('should handle webhook failure with 202 and outbox', async () => {
            ...
            const response = await (0, supertest_1.default)(casinoApp)
                ...
                newBalance: '1000.00',
                oldBalance: '990.00',
                transactionRef: (0, uuid_1.v4)()
            });
            // Should return 202 when forward fails (no admin URL configured)
            expect(response.status).toBe(202);
            // Verify outbox entry created
            const { query } = require('../config/database');
            const outboxRows = await query('SELECT * FROM webhook_outbox WHERE player_id = ? ORDER BY created_at DESC LIMIT 1', [testPlayerId]);
    ...
    describe('TASK C - Health & Degradation', () => {
        it('should return healthy status normally', async () => {
            ...
            for (const endpoint of healthEndpoints) {
                const response = await (0, supertest_1.default)(casinoApp).get(endpoint);
                expect(response.status).toBe(200);
        ...
        it('should return 503 on DB failure', async () => {
            ...
            const response = await (0, supertest_1.default)(casinoApp).get('/casino/api/v1/health');
    ...
    describe('TASK D - GDPR Logging', () => {
        it('should log only whitelisted fields', async () => {
            ...
            const transactionLog = logSpy.mock.calls.find(call => call[0] === 'Transaction debit processed');
        ...
        it('should handle 100 concurrent requests with measured latency', async () => {
            ...
            for (let i = 0; i < totalRequests; i += concurrency) {
                ...
                for (let j = 0; j < concurrency && i + j < totalRequests; j++) {
                    ...
                    batch.push((0, supertest_1.default)(rgsApp)
                        .post('/rgs/api/v1/transaction/debit')
                        .set('Authorization', 'Bearer test_rgs_key')
                        .set('X-Correlation-Id', (0, uuid_1.v4)())
                        .set('Idempotency-Key', (0, uuid_1.v4)())
...
//# sourceMappingURL=E2E_rgs_casino.test.js.map